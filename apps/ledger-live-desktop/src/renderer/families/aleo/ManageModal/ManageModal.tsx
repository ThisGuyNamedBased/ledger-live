...
// ToolTip's wrapper is inline-flex, which would otherwise shrink the full-width button.
...
const ManageModal = ({ account, parentAccount, source, ...rest }: Data) => {
  ...
  // `unbond_public` and `claim_unbond_public` both rewrite the staker's single `unbonding`
  // slot, so either one in flight closes both actions — not just its own. The balances these
  // gates read also still describe the pre-transaction chain state until the next sync.
  ...
  // Name the operation that is actually holding the slot, rather than the button's own.
